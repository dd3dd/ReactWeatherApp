...
export default function WeatherData({ weatherData, showText }) {
    //  console.log(showText)
    return (
        <div className='WeatherData'>
            <p className={`fade-effect ${showText ? 'show' : ''} Hours`}>
                {weatherData.length !== 0 && `${weatherData[5]}`}
            </p>
            <p className={`fade-effect ${showText ? 'show' : ''} StartEndTime`}>
                {weatherData.length !== 0 && `${weatherData[0]}~${weatherData[1]}`}
            </p>
            <p className={`fade-effect ${showText ? 'show' : ''} `}>
